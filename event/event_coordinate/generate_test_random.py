...
def drag_event(driver):
    ...
    TouchAction(driver).long_press(el=None, x=from_x, y=from_y).move_to(el=None, x=to_x, y=to_y).release().perform()
...
def key_event(driver):
    ...
    # 91：话筒静音键(no)
    # 164：扬声器静音键(no)
    keycode_list = [3, 4, 24, 25, 26, 82]
    ran = random.random()
    if(ran<0.8):
        select_keycode = keycode_list[1]
    else:
...
def long_press_event(driver):
    ...
    TouchAction(driver).long_press(el= None,x=x, y=y).release().perform()
...
#     x = random.randint(0, width)
#     y = random.randint(0, height)
#     TouchAction(driver).press(el=None, x=x, y=y).release().perform()
#     # swipe event
#     from_x = random.randint(0, width)
#     from_y = random.randint(0, height)
#     to_x = random.randint(0, width)
...
def pinch_event(driver):
    ...
    height = driver.get_window_size()['height']
    # pinch
    action1 = TouchAction(driver).press(el=None, x=width * 0.2, y=height * 0.2).wait(1000).move_to(el=None, x=width * 0.4,
                                                                                      y=height * 0.4).release()
    action2 = TouchAction(driver).press(el=None, x=width * 0.8, y=height * 0.8).wait(1000).move_to(el=None, x=width * 0.6,
...
def zoom_event(driver):
    ...
    height = driver.get_window_size()['height']
    # zoom
    action1 = TouchAction(driver).press(el=None, x=width * 0.4, y=height * 0.4).wait(1000).move_to(el=None, x=width * 0.2,
                                                                                      y=height * 0.2).release()
    action2 = TouchAction(driver).press(el=None, x=width * 0.6, y=height * 0.6).wait(1000).move_to(el=None, x=width * 0.8,
...
#         action = TouchAction(driver).press(el=None, x=from_coordidate[0], y=from_coordidate[1]). \
#             wait(1000).move_to(el=None, x=to_coordinate[0], y=to_coordinate[1]).release()
...
def random_test(driver):
    ...
    event_list = ["drag", "key", "long_press", "motion", "multi_touch", "throttle"]
