...
def run_cases_on_image(apk_path, replay_file_list, image_name):
    ...
    try:
        ...
        try:
            ...
            print(case.stem, e)
        return write_name
        log_to_file(f"/home/luzhirui/jerrylu/testAppium/replay_diff_screenshots/logs/replay.log_{write_name}", "adbinstall:"+str(e),now, write_name)
    ...
    for idx, case in enumerate(replay_file_list):
        ...
        code += """\n
    with open(f"replay_diff_screenshots/{write_name}.png", "wb") as f:
        ...
        try:
            ...
            log_to_file(f"/home/luzhirui/jerrylu/testAppium/replay_diff_screenshots/logs/replay.log_{write_name}", "SUCCESS!", now, write_name)
        except Exception as e:
            print(case.stem, e)
            exc_type, exc_value, exc_traceback_obj = sys.exc_info()
            traceback.print_tb(exc_traceback_obj)
            log_to_file(f"/home/luzhirui/jerrylu/testAppium/replay_diff_screenshots/logs/replay.log_{write_name}", "ERR:"+str(e), now, write_name)
            if "socket hang up" in str(e):
                ...
                try:
                    ...
                    log_to_file(f"/home/luzhirui/jerrylu/testAppium/replay_diff_screenshots/logs/replay.log_{write_name}", "SUCCESS! with retry", now, write_name)
                except Exception as e:
                    print(case.stem, e)
                    log_to_file(f"/home/luzhirui/jerrylu/testAppium/replay_diff_screenshots/logs/replay.log_{write_name}", "ERR(retry):"+str(e), now, write_name)
        # finally:
    try:
        container.remove(force=True)
        print("{*} container removed.")
        ...
        print(case.stem, e)
        log_to_file(f"/home/luzhirui/jerrylu/testAppium/replay_diff_screenshots/logs/replay.log_{write_name}", "ERR(final):"+str(e), now, write_name)
    finally:
...
diff_json_path = "/home/luzhirui/jerrylu/testAppium/ipynbs/page_source_diff_remaining3.json"
with open(diff_json_path, "r") as f:
    diffs = json.load(f)

apk_root = Path("/home/luzhirui/fdroid_1k6")

for idx, diff in enumerate(diffs):

    apk_name = diff["apk_name"]
    cn_arr = diff["test_name"].split("_t")
    ctest_no = int(cn_arr[0][1:])
    test_no = int(cn_arr[1])

    apk_path = apk_root / diff["apk_name"]
    ...
    try:
        ...
        #             content = f.readlines()
        #         if not only_sleep1(content) or len(content) <= 5:
        #             replay_file_list.append(file)
        #         else:
        #             print("skipping "+str(file))

        replay_file_list.append(Path(f"/home/luzhirui/jerrylu/testAppium/replay/output/testcase_{apk_name}-insted_ctest_{ctest_no}_test{test_no}.py"))

        print(idx, replay_file_list)
        ...
        with open("screenshot.log", "a") as f:
            f.write(diff)
            f.write("\n")
            f.write(e)
            f.write("\n-------\n")
